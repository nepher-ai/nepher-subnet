...
if ! docker image inspect "${SANDBOX_IMAGE}" &>/dev/null; then
    echo "[WARNING] Sandbox image '${SANDBOX_IMAGE}' not found locally."
    echo "          Build it with: docker build -f docker/Dockerfile.sandbox -t ${SANDBOX_IMAGE} ."
    echo "          The validator will attempt to use it when evaluation starts."
fi
